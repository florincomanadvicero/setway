...
if (!function_exists('sports_store_system_fonts_list')) {
	function sports_store_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('sports_store_text_decoration_list')) {
	function sports_store_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'sports-store'), 
			'underline' => esc_html__('underline', 'sports-store'), 
			'overline' => esc_html__('overline', 'sports-store'), 
			'line-through' => esc_html__('line-through', 'sports-store'), 
		);
...
if (!function_exists('sports_store_custom_color_schemes_list')) {
	function sports_store_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'sports-store'), 
			'second' => esc_html__('Custom 2', 'sports-store'), 
			'third' => esc_html__('Custom 3', 'sports-store') 
		);
		
		
		return apply_filters('sports_store_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('sports_store_locate_template')) {
	function sports_store_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Yith WooCommerce Compare Plugin Support Constants
...
define('CMSMASTERS_WCCL', class_exists('YITH_WCCL') && defined('YITH_WCCL_PREMIUM') && CMSMASTERS_WOOCOMMERCE ? true : false);

// Yith WooCommerce Ajax Product Filter Plugin Support Constants
define('CMSMASTERS_WCAN', class_exists('YITH_WCAN') && CMSMASTERS_WOOCOMMERCE ? true : false);
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_sports-store_theme_style') ? get_option('cmsmasters_sports-store_theme_style') : ''));
...
if (!function_exists('sports_store_all_theme_styles')) {
	function sports_store_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('sports_store_all_color_schemes_list')) {
	function sports_store_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'sports-store'), 
			'header' => 		esc_html__('Header', 'sports-store'), 
			'navigation' => 	esc_html__('Navigation', 'sports-store'), 
			'header_top' => 	esc_html__('Header Top', 'sports-store'), 
			'footer' => 		esc_html__('Footer', 'sports-store') 
		);
		
		
		$out = array_merge($list, sports_store_custom_color_schemes_list());
...
if (CMSMASTERS_WCCL) {
	require_once(get_template_directory() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/yith-woocommerce-color-label-variations/cmsmasters-plugin-functions.php');
}

// Yith WooCommerce Ajax Product Filter functions
if (CMSMASTERS_WCAN) {
	require_once(get_template_directory() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/yith-woocommerce-ajax-product-filter/cmsmasters-plugin-functions.php');
}


// Load Theme Local File
if (!function_exists('sports_store_load_theme_textdomain')) {
	function sports_store_load_theme_textdomain() {
		load_theme_textdomain('sports-store', CMSMASTERS_VARS . '/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'sports_store_load_theme_textdomain')) {
	add_action('after_setup_theme', 'sports_store_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('sports_store_theme_activation')) {
	function sports_store_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'sports-store') {
			add_option('cmsmasters_active_theme', 'sports-store', '', 'yes');
...
add_action('after_switch_theme', 'sports_store_theme_activation');



// Framework Deactivation
if (!function_exists('sports_store_theme_deactivation')) {
	function sports_store_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'sports_store_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('sports_store_plugin_activation')) {
	function sports_store_plugin_activation($plugin, $network_activation) {
...
add_action('activated_plugin', 'sports_store_plugin_activation', 10, 2);


if (!function_exists('sports_store_plugin_activation_regenerate')) {
	function sports_store_plugin_activation_regenerate() {
...
add_action('init', 'sports_store_plugin_activation_regenerate');


function sports_store_run_reinit_import_options($post_id, $key, $value) {
	if (!get_post_meta($post_id, 'cmsmasters_heading', true)) {
		$custom_post_meta_fields = sports_store_get_custom_all_meta_fields();
...
add_action('import_post_meta', 'sports_store_run_reinit_import_options', 10, 3);
