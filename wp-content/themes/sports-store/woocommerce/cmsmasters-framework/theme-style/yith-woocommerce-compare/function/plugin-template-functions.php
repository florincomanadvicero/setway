...
function sports_store_compare_button($type = '') {
	global $product;
	...
	if ( ! isset( $button_text ) || $button_text == 'default' ) {
		$button_text = get_option( 'yith_woocompare_button_text', __( 'Compare', 'sports-store' ) );
	...
	printf( '<a href="%s" class="%s cmsmasters_compare_btn" data-product_id="%d" rel="nofollow" title="' . esc_attr__('Compare', 'sports-store') . '">%s</a>', sports_store_compare_add_product_url( $product_id ), 'compare' . ( $is_button == 'button' ? ' button' : '' ), $product_id, $button_text );
}


/* Yith WooCommerce URL to add the product into the comparison table */
function sports_store_compare_add_product_url( $product_id ) {
