...
function sports_store_yith_woocommerce_quick_view_register_styles_scripts() {

	wp_enqueue_style('sports-store-yith-woocommerce-quick-view-style', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/yith-woocommerce-quick-view/css/plugin-style.css', array(), '1.0.0', 'screen');
	
	wp_enqueue_style('sports-store-yith-woocommerce-quick-view-adaptive', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/yith-woocommerce-quick-view/css/plugin-adaptive.css', array(), '1.0.0', 'screen');
	
	if (is_rtl()) {
		wp_enqueue_style('sports-store-yith-woocommerce-quick-view-rtl', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/yith-woocommerce-quick-view/css/plugin-rtl.css', array(), '1.0.0', 'screen');
	}
	
}

add_action('wp_enqueue_scripts', 'sports_store_yith_woocommerce_quick_view_register_styles_scripts');



/* Yith WooCommerce Quick View Default */
function sports_store_quick_view_options_default() {
	if (!get_option('cmsmasters_sports_store_quick_view_first_active')) {
		...
		add_option('cmsmasters_sports_store_quick_view_first_active', 'true');
	}
}

add_action('init', 'sports_store_quick_view_options_default');



/* Yith WooCommerce Quick View Premium Default */
if (defined('YITH_WCQV_PREMIUM')) {
	function sports_store_quick_view_premium_options_default() {
		if (!get_option('cmsmasters_sports_store_quick_view_premium_first_active')) {
			update_option('yith-wcqv-background-modal', '');
			...
			add_option('cmsmasters_sports_store_quick_view_premium_first_active', 'true');
		}
	}
	
	add_action('init', 'sports_store_quick_view_premium_options_default');
}