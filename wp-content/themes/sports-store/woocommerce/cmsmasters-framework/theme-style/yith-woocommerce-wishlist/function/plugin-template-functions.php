...
function sports_store_add_wishlist_button($show = true) {
	global $product;
	...
	$popup_title = apply_filters('yith_wcwl_add_to_wishlist_popup_title', __('Select a Wishlist', 'sports-store'));
		...
		if (!($disable_wishlist && ! is_user_logged_in())):
			$out .= '<div class="yith-wcwl-add-button ' . (($exists && ! $available_multi_wishlist) ? 'hide': 'show') . '" style="display:' . (($exists && ! $available_multi_wishlist) ? 'none': 'block') . '">';
				if (!$available_multi_wishlist) {
					$out .= '<a href="' . esc_url(add_query_arg('add_to_wishlist', $product_id)) . '" rel="nofollow" data-product-id="' . $product_id . '" data-product-type="' . $product_type . '" class="button cmsmasters_theme_icon_wishlist ' . $link_classes . '" title="' . esc_attr__('Wishlist', 'sports-store') . '"></a>' . 
					...
					if ($show_exists && $exists): 
						...
						$out .= '<a href="#add_to_wishlist_popup_' . $product_id . '_' . $unique_id . '" rel="nofollow" class="button cmsmasters_theme_icon_wishlist ' . $link_classes . ' open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_' . $product_id . '_' . $unique_id . ']" title="' . esc_attr__('Wishlist', 'sports-store') . '"></a>' . 
								...
								'<div class="yith-wcwl-popup-content">' . 
									'<div class="yith-wcwl-first-row">' . 
										'<div class="yith-wcwl-wishlist-select-container">' . 
											...
											'<select name="wishlist_id" class="wishlist-select">' . 
												'<option value="0">' . apply_filters('yith_wcwl_default_wishlist_name', __('My Wishlist', 'sports-store')) . '</option>';
													if(!empty($lists)): 
												...
												$out .= '<option value="new">' . apply_filters('yith_wcwl_create_new_list_text', __('Create a new list', 'sports-store')) . '</option>' . 
											'</select>' . 
										'</div>' . 
									...
									'<div class="yith-wcwl-second-row">' . 
										'<div class="yith-wcwl-popup-new">' . 
											'<label for="wishlist_name">' .  apply_filters('yith_wcwl_new_list_title_text', __('Wishlist name', 'sports-store')) . '</label>' . 
										...
										'<div class="yith-wcwl-visibility">' . 
											'<select name="wishlist_visibility" class="wishlist-visibility">' . 
												'<option value="0" class="public-visibility">' . apply_filters('yith_wcwl_public_wishlist_visibility', __('Public', 'sports-store')) . '</option>' . 
												'<option value="1" class="shared-visibility">' . apply_filters('yith_wcwl_shared_wishlist_visibility', __('Shared', 'sports-store')) . '</option>' . 
												'<option value="2" class="private-visibility">' . apply_filters('yith_wcwl_private_wishlist_visibility', __('Private', 'sports-store')) . '</option>' . 
											'</select>' . 
										'</div>' . 
									'</div>' . 
								'</div>' . 
			...
			'<div class="yith-wcwl-wishlistaddedbrowse hide" style="display:none;">' . 
				'<a href="' . esc_url($wishlist_url) . '" class="button browse_wishlist cmsmasters_theme_icon_wishlist" rel="nofollow" title="' . esc_attr__('Wishlist', 'sports-store') . '"></a>' . 
			'</div>' . 
...
/* Wishlist Count */
function sports_store_shop_wishlist_count() {
	$wishlist_url = YITH_WCWL()->get_wishlist_url();
