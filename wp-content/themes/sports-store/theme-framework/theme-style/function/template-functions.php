...
/* Post Type Video */
function sports_store_post_type_video($cmsmasters_id, $video_type = '', $video_link = '', $video_links = '', $size = 'post-thumbnail') {
	if (!post_password_required()) {
		if ($video_type == 'selfhosted' && !empty($video_links) && sizeof($video_links) > 0) {
			...
			sports_store_thumb($cmsmasters_id, $size, true, false, true, false, true, true, false);
		}
	}
}
...
/* Post Type Slider */
function sports_store_post_type_slider($cmsmasters_id, $images, $size = 'post-thumbnail', $slider_data = '') {
	if (!post_password_required()) {
		if (sizeof($images) > 1) {
			echo '<div' . 
			...
			sports_store_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, $images[0]);
		} elseif (has_post_thumbnail()) {
			sports_store_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, false);
		}
	}
}
...
/* Post Type Gallery */
function sports_store_post_type_gallery($cmsmasters_id, $images, $columns, $size_col = 'post-thumbnail', $size_full = 'post-thumbnail') {
	if (!post_password_required()) {
		...
		if (sizeof($images) > 0 && $images[0] != '') {
			...
			sports_store_thumb($cmsmasters_id, $size_full, false, 'img_' . $cmsmasters_id, true, true, false, true, false);
		}
	}
}
...
/* Post Type Image */
function sports_store_post_type_image($cmsmasters_id, $image_link = '', $size = 'cmsmasters-masonry-thumb') {
	if (!post_password_required()) {
		if ($image_link != '') {
			sports_store_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, $image_link);
		} elseif (has_post_thumbnail()) {
			sports_store_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, false);
		}
	}
}
...
/* Post Type Audio */
function sports_store_post_type_audio($audio_links) {
...
function sports_store_get_header_search_form($cmsmasters_option) {
	if (
		$cmsmasters_option['sports-store' . '_header_search'] && 
		$cmsmasters_option['sports-store' . '_header_styles'] != 'c_nav'
	) {
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				<button type=\"submit\" class=\"cmsmasters_theme_icon_search\"></button>
				<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__('Enter Keywords', 'sports-store') . "\" value=\"\" />
			</div>
		</form>";
		
		
		$out_form = apply_filters('sports_store_get_header_search_form_filter', $out_form);
...
/* Theme Error Styles */
function sports_store_theme_error_styles() {
	$out = "";
	...
	if (is_404()) {
		...
		if (
			$cmsmasters_option['sports-store' . '_error_bg_img_enable'] && 
			$cmsmasters_option['sports-store' . '_error_bg_image'] != ''
		) {
			$error_bg_image = explode('|', $cmsmasters_option['sports-store' . '_error_bg_image']);
			...
			$out .= "
	.error .error_bg {
		background-image : " . (!empty($cmsmasters_option['sports-store' . '_error_bg_image']) ? 'url(' . ((is_numeric($error_bg_image[0])) ? $error_bg_image_url[0] : $error_bg_image[1]) . ')' : 'none') . ";
		background-position : " . (!empty($cmsmasters_option['sports-store' . '_error_bg_pos']) ? $cmsmasters_option['sports-store' . '_error_bg_pos'] : 'top center') . ";
		background-repeat : " . (!empty($cmsmasters_option['sports-store' . '_error_bg_rep']) ? $cmsmasters_option['sports-store' . '_error_bg_rep'] : 'repeat') . ";
		background-attachment : " . (!empty($cmsmasters_option['sports-store' . '_error_bg_att']) ? $cmsmasters_option['sports-store' . '_error_bg_att'] : 'scroll') . ";
		background-size : " . (!empty($cmsmasters_option['sports-store' . '_error_bg_size']) ? $cmsmasters_option['sports-store' . '_error_bg_size'] : 'auto') . ";
	}
";
		}
		...
		$out .= "
	.error .error_bg {
		" . cmsmasters_color_css('background-color', $cmsmasters_option['sports-store' . '_error_bg_color']) . "
	}
	
	.error .error_title, 
	.error .error_subtitle {
		" . cmsmasters_color_css('color', $cmsmasters_option['sports-store' . '_error_color']) . "
	}
";
	}
...
add_action('wp_enqueue_scripts', 'sports_store_theme_error_styles');



/* Get Page Heading Function */
function sports_store_page_heading() {
	if (is_404() || is_home()) {
		...
		if (
			...
			$cmsmasters_header_overlaps = $cmsmasters_option['sports-store' . '_header_overlaps'] ? 'true' : 'false';
			
			$cmsmasters_heading_alignment = $cmsmasters_option['sports-store' . '_heading_alignment'];
			$cmsmasters_heading_scheme = $cmsmasters_option['sports-store' . '_heading_scheme'];
			
			$cmsmasters_breadcrumbs = $cmsmasters_option['sports-store' . '_breadcrumbs'] ? 'true' : 'false';
		}
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('%1$d search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'sports-store')), $wp_query->found_posts, get_search_query());
						} else {
							echo sprintf(esc_html__('0 search results for: %s', 'sports-store'), get_search_query());
						}
					} elseif (is_archive()) {
						if (is_author()) {
							if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
								echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'sports-store'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
							} else {
								echo sprintf(esc_html__('Author: %s', 'sports-store'), '<span class="vcard">' . get_the_author() . '</span>');
							}
						} else {
...
function sports_store_theme_heading_styles() {
	$cmsmasters_option = sports_store_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['sports-store' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['sports-store' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['sports-store' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['sports-store' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['sports-store' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['sports-store' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['sports-store' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'sports_store_theme_heading_styles');



/* Get Social Icons Styles Function */
function sports_store_theme_social_icons_styles() {
...
add_action('wp_enqueue_scripts', 'sports_store_theme_social_icons_styles');



/* Get Posts Thumbnail Function */
function sports_store_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function sports_store_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
function sports_store_theme_category_styles() {
	$out = "";
	
	
	if (CMSMASTERS_COLORED_CATEGORIES) {
		$args = sports_store_colored_categories_args();
...
add_action('wp_enqueue_scripts', 'sports_store_theme_category_styles');



/* Get Page Container Classes */
function sports_store_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	...
	echo 'cmsmasters_' . $cmsmasters_option['sports-store' . '_theme_layout'] . ' ';
	
	
	if ($cmsmasters_option['sports-store' . '_fixed_header']) {
		echo 'fixed_header ';
	}
	
	
	if ($cmsmasters_option['sports-store' . '_header_top_line']) {
	...
	if ($cmsmasters_option['sports-store' . '_header_styles'] == 'r_nav') {
		echo 'enable_header_right ';
	}
	
	
	if ($cmsmasters_option['sports-store' . '_header_styles'] == 'c_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['sports-store' . '_header_overlaps'];
	}
...
/* Theme Header Styles */
function sports_store_theme_header_styles() {
	$cmsmasters_option = sports_store_get_global_options();
	
	$defaults = sports_store_settings_general_defaults();
	
	
	$header_top_height = (($cmsmasters_option['sports-store' . '_header_top_height'] !== '') ? $cmsmasters_option['sports-store' . '_header_top_height'] : $defaults[$tab]['sports-store' . '_header_top_height']);
	...
	$header_bot_height = (($cmsmasters_option['sports-store' . '_header_bot_height'] !== '') ? $cmsmasters_option['sports-store' . '_header_bot_height'] : $defaults[$tab]['sports-store' . '_header_bot_height']);
...
add_action('wp_enqueue_scripts', 'sports_store_theme_header_styles');
