...
function sports_store_prev_next_posts($order_cat = false, $tax_name = 'category') {
	$cmsmasters_post_type = get_post_type();

	$published_posts = wp_count_posts($cmsmasters_post_type)->publish;

	if ( 'post' === $cmsmasters_post_type ) {
		$post_type = 'Post';
	} elseif ( 'project' === $cmsmasters_post_type ) {
		$post_type = 'Project';
	} elseif ( 'profile' === $cmsmasters_post_type ) {
		$post_type = 'Profile';
	}
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'sports-store') . '<span class="post_nav_type"> ' . __($post_type, 'sports-store') . ' </span>' . esc_html__('Link', 'sports-store') . '</span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'sports-store') . '<span class="post_nav_type"> ' . __($post_type, 'sports-store') . ' </span>' . esc_html__('Link', 'sports-store') . '</span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
...
function sports_store_sharing_box($title_box = false, $tag = 'h3') {
...
function sports_store_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'sports-store') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function sports_store_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'sports-store')) . 
				"</" . esc_html($tag) . ">" . 
 						...
 						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									sports_store_thumb(get_the_ID(), 'cmsmasters-blog-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function sports_store_author_avatar($template_type = 'page') {
...
function sports_store_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'sports-store') . '</' . esc_html($tag) . '>' . "\n" .
