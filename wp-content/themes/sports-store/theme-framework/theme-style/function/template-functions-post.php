...
function sports_store_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function sports_store_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function sports_store_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
...
function sports_store_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<span class="cmsmasters_post_author_suf">' . esc_html__('by', 'sports-store') . ' </span>' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'sports-store') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
		...
		if ($cmsmasters_option['sports-store' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
...
function sports_store_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out .= '<span class="cmsmasters_post_category">' . 
				'<span class="cmsmasters_post_cat_suf">' . esc_html__('in', 'sports-store') . ' </span>' . 
			...
			if ($cmsmasters_option['sports-store' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
...
function sports_store_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = sports_store_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['sports-store' . '_blog_post_tag']) {
			$out .= '<span class="cmsmasters_post_tags">' . 
				get_the_tag_list(esc_html__('tags', 'sports-store') . ' ', ', ', '') . 
			'</span>';
		}
...
function sports_store_post_exc_cont($length = 20, $show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
		wpautop(sports_store_excerpt($length, false)) . 
	'</div>' . "\n");
	
	
	if (sports_store_excerpt($length, false) != '') {
		if ($show) {
...
function sports_store_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['sports-store' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function sports_store_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = sports_store_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = sports_store_get_global_options();
			
			if ($cmsmasters_option['sports-store' . '_blog_post_comment']) {
				$out = sports_store_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function sports_store_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Read More', 'sports-store');
	}
...
function sports_store_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
